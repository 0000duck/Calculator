...
namespace BestCalculator
{
    public class ExpressionAndAnswer   //数据运算和处理类
    {
        ...
        private static double Parse(string expression)      //计算后缀表达式的值
        {
        ...
        public String getAnswer()//将计算结果回传
        {
            double answer = Parse(expression);
        ...
        public string getLastOperate()
        {
            ...
            {
                lastOperate = lastOperate.Insert(0, expression[i].ToString());
                i--;
            }
            lastOperate = lastOperate.Insert(0, expression[i].ToString());
            return lastOperate;
